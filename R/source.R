...
#'
#' Tests whether both `lat` and `lon` columns are present and whether at least
#' one row contains numeric, non-missing values for both coordinates.
#'
#' @param dta A data frame containing uploaded or standardized data.
#'
...
#' Read uploaded data
#'
#' Reads an uploaded CSV, XLS, or XLSX file using the same parsing rules as the
#' original Shiny application. CSV files are read with semicolon separators.
#'
#' @param path Character string. Path to the temporary uploaded file.
#' @param filename Character string. Original uploaded file name.
#'
#' @return A data frame.
#' @export
...
#'
#' Renames selected metadata columns to the standard names used internally by the
#' application and keeps numeric genetic columns unchanged.
#'
#' @param dta A data frame with uploaded data.
#' @param col_map A data frame produced by [make_column_map()] with columns
#'   `col_names` and `standard_col_names`.
#'
...
#'
#' Creates the mapping between user-selected column names and the standardized
#' internal names used by the application.
#'
#' @param pop_col Character string. Population column name.
#' @param group_col Character string. Group column name.
#' @param sample_col Optional character string. Sample column name, or `None`.
#' @param lon_col Optional character string. Longitude column name, or `None`.
#' @param lat_col Optional character string. Latitude column name, or `None`.
#'
#' @return A data frame with original and standardized column names.
#' @export
...
#'
#' Separates standardized metadata columns from numeric genetic columns.
#'
#' @param dta A standardized data frame.
#'
#' @return A list with `matriz_genetica`, `pop_info`, and `genetic_cols`.
#' @export
...
#'
#' Combines private/shared allele summaries by group with R coefficients and
#' computes conservation priority scores.
#'
#' @param group_summary A data frame with PSA results by allele and group.
#' @param rtable A data frame with R values by group.
#'
#' @return A data frame with PSA, R, relative R, and priority scores.
#' @export
...
#'
#' Returns the first rows and columns of a data frame for display in the Shiny
#' application.
#'
#' @param dta A data frame.
#' @param max_rows Maximum number of rows to include.
#' @param max_cols Maximum number of columns to include.
#'
...
#'
#' @param patterns Character vector of candidate patterns.
#' @param vector Character vector to search.
#'
#' @return A character string with the first match, or `None` if no match exists.
#' @export
...
# -----------------------------------------------------------------------------  
# Funcion para seleccionar alelos de baja frecuencia 
#' Select low-frequency alleles
#'
#' Selects alleles according to maximum individual allele frequency and maximum
#' population frequency thresholds.
#'
#' @param mat Numeric matrix or data frame of genetic data.
#' @param pops Population identifier for each row of `mat`.
#' @param allele_perc Maximum overall allelic frequency.
#' @param pop_perc Maximum frequency across populations.
#'
#' @return A character vector with selected allele names.
#' @export
...
# -----------------------------------------------------------------------------  
# Funcion interna para calcular indices geneticos (.compute_genetic_indices)
#' Compute genetic indices for selected alleles
#'
#' Internal helper that calculates allele frequencies, population counts, Lo, Le,
#' log-transformed indices, R coefficient, and the associated plot.
#'
#' @param mat Numeric matrix or data frame of genetic data.
#' @param pops Population identifier for each row of `mat`.
#' @param alleles Character vector of allele column names to analyze.
#'
#' @return A list with `table`, `R`, and `plot`.
#' @keywords internal
.compute_genetic_indices <- function(mat, pops, alleles) {
  
  validate(
    need(length(alleles) > 0, "No hay alelos validos para analizar"),
  ...
  if (ncol(allele_matrix) == 0) {
    stop("Ningun alelo valido disponible despues de la seleccion")
  }
  ...
  lm_logLo <- stats::lm(logLo ~ allele_percentage)
  lm_logLe <- stats::lm(logLe ~ allele_percentage)
  
  R_coef <- stats::coef(lm_logLo)["allele_percentage"] / stats::coef(lm_logLe)["allele_percentage"]
  names(R_coef) <- ""
  ...
  plot_obj <- ggplot2::ggplot(table_long, ggplot2::aes(x = allele_perc, y = value, color = name)) +
    ggplot2::geom_point() +
...
# -----------------------------------------------------------------------------  
# Funcion para calcular el PSA de cada alelo
#' Calculate private shared allele percentages
#'
#' Internal helper that computes sample-level and population-level PSA summaries
#' for each group.
#'
#' @param mat Numeric matrix or data frame of genetic data.
#' @param pops Population identifier for each row of `mat`.
#' @param groups Group identifier for each row of `mat`.
#' @param alleles Character vector of allele column names to analyze.
#'
#' @return A data frame with PSA values and the group of maximum PSA per allele.
#' @keywords internal
...
# -----------------------------------------------------------------------------  
# Funcion para ejecutar el analisis global y por grupos.
...
#' @param allele_perc Maximum overall allelic frequency.
#' @param pop_perc Maximum frequency across populations.
#' @param Fst FST value used to calculate the probability of retaining rare alleles.
#' @param groups Optional group identifier for each row of `mat`.
#'
#' @return A list with global results, group results, PSA results, and needed populations.
#' @export
...
# -----------------------------------------------------------------------------  
#' Format R coefficients table
#'
#' Internal helper that extracts and rounds R coefficients from RGUC results.
#'
#' @param res A result list returned by [RGUC()].
#'
#' @return A data frame with group names and R coefficients.
#' @keywords internal
...
# -----------------------------------------------------------------------------  
# Funcion para extraer y formatear los graficos por grupo
#' Extract group plots
#'
#' Internal helper that adds group names as plot titles to group-level plots.
#'
#' @param resultados_grupos A named list of group-level RGUC results.
#'
...
# -----------------------------------------------------------------------------  
# Funcion para calcular el numero de poblaciones y la Probabilidad de perdida de alelos raros.
#' Calculate needed populations
#'
#' Internal helper that calculates the cumulative probability of retaining rare
#' alleles as the number of populations increases.
#'
#' @param Fst FST value.
#' @param nmax Maximum number of populations to evaluate.
#'
#' @return A data frame with population number and retention probability.
#' @keywords internal
...
# -----------------------------------------------------------------------------  
# Funcion principal para procesar el analisis genetico de forma generica.
#' Process a complete genetic analysis
#'
#' Main wrapper used by the Shiny application to run the RGUC workflow and format
#' the objects required by the outputs.
#'
#' @param matriz Numeric matrix or data frame of genetic data.
#' @param pop_info Data frame containing at least a `pop` column and optionally a
#'   `group` column.
#' @param allele_perc Maximum overall allelic frequency.
#' @param pop_perc Maximum frequency across populations.
#' @param Fst FST value used to calculate the probability of retaining rare alleles.
#'
#' @return A list with selected alleles, plots, R table, PSA summary, and needed populations.
#' @export
genetic_process <- function(matriz, pop_info, allele_perc = 0.1, pop_perc = 0.1, Fst = 0.099) {
  ...
  # Extraer graficos por grupo y calcular tablas adicionales
  ...
  list(
    ...
    plot_global = resultados$global$plot,  # Grafico global
    plots_grupos = plots_grupos,            # Graficos por grupo
